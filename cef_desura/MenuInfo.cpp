...
{
	if (m_LinkUrl.empty())
		m_LinkUrl = m_MenuInfo->GetLinkUrl();
...
{
	if (m_ImgUrl.empty())
		m_ImgUrl = m_MenuInfo->GetSourceUrl();
...
{
	if (m_PageUrl.empty())
		m_PageUrl = m_MenuInfo->GetPageUrl();
...
{
	if (m_FrameUrl.empty())
		m_FrameUrl = m_MenuInfo->GetFrameUrl();
...
{
	if (m_SelText.empty())
		m_SelText = m_MenuInfo->GetSelectionText();
